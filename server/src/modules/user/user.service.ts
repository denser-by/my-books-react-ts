...
export class UserService {
    ...
    private async prepareDtoDelete(ref: any): Promise<DeleteUserDto> {
        let result = new DeleteUserDto();
        result.id = ref.id;
        result.login = ref.login;
        return result;
    }

    async delete(id: number): Promise<DeleteUserDto> {
        if (id == null || id == undefined || id < 0)
            throw new Error('Не указан ID');
        var { count, rows } = await user.findAndCountAll({ where: { id: id } });
        if (count != 1)
    ...
    async deleteAll(): Promise<DeleteUserDto[]> {
        var { count, rows } = await user.findAndCountAll({});
        if (count < 1)
